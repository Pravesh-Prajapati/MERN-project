...
const Home = () => {
  ...
  useEffect(() => {
    const filteredRecipe = async () => {
      let res = await axios.get("http://localhost:3456/getCategory")
  ...
  useEffect(() => {
    const allRecipe = async () => {
      let res = await axios.get(`http://localhost:3456/getRecipe?category=${searchTerm}`)
  ...
  useEffect(() => {
    try {
      let getIsVeg = async () => {
        let res = await axios.get(`http://localhost:3456/getIsVegetarian?isVegetarian=${isVegSearch}`);
        if (res.data) {
  ...
  const handleFavourite = async (id) => {
    try {
      if (user === null) {
        ...
        const res = await axios.get(`http://localhost:3456/selectFavourite/${id}`, {
